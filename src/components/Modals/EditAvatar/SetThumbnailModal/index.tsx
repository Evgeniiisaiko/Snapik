...
import { resetEditAvatar, setCrop, setPreviewCrop, setStep } from '../../../../features/edit-avatar/main';
...
import { getImage } from '../../../../utils/editAvatar/getImageFromCanvas';
  ...
  return (
    <div className={"f-column gap-30 f-1"}>
      ...
      {!err && imgSrc && crop !== null ?
        <div className={`w-100p f-c-col ${styles.imageArea}`}>
          <ReactCrop
            ...
            <img style={{ maxHeight: 400 }} ref={imgRef} onLoad={e => handleImageLoad(e)} src={imgSrc} />
          </ReactCrop>
      ...
      <div className="w-100p f-1 f-row-betw gap-30">
        <UnfilledButton onClick={handlePrevStep} className={`f-1 ${styles.backbtn}`} title={"Назад"} />
        <UnfilledButton disabled={previewCrop === null} onClick={sendAvatarForm} className={"f-1"} title={"Сохранить"} />
      </div>
    </div>
  )
}
