import React, { FC, HTMLInputTypeAttribute, useState, useEffect } from 'react';
...
import { handleOrder, setBooking } from "../../../features/modals/modalsSlice";
import { ExcursionApi, ExcursionProps } from '../../../types/excursions.types';
import { OrderFormData } from '../../../types/user.types';
import { FormChangeValByKey, handleOrderForm, resetOrderForm, setOrderForm } from '../../../features/forms/formsSlice';
...
const OrderInput: FC<OrderInputProps> = ({ keyField, placeholder, val, typeInput }) => {
    ...
    if (keyField == "phone") {
        return <InputMask value={val} mask={"+9(999)999-99-99"} placeholder={"+7"} onChange={e =>
            dispatch(handleOrderForm({
                keyField: keyField,
                val: e.target.value
            }))} />
    }
    return <input type={typeInput} onChange={e => dispatch(handleOrderForm({
        keyField: keyField,
        val: e.target.value
...
type OrderModalProps = Pick<ExcursionApi, "id" | "people_limit" | "title" | "tour_price" | "cost" | "payment_type" | "price_variant">
                ...
                {orderStatus.success ?
                    <>
                        <div className="f-column gap-20 al-center txt-center">
                            ...
                            <SuccessGreenIcon />
                            <UnfilledButton onClick={handleToMyBookings} title={"Мои бронирования"} />
                        </div>
                    </> :
                    <>
                        ...
                        <div className={`${styles.form} f-1 gap-20 f-column`}>
                            <div className={`f-row-betw gap-20 ${styles.orderFields}`}>
                                <InputField wrapperClass={`bx-shadow d-f al-end w-100p pd-20 `} className={"w-100p"}
                                    inputId={"s"}
                                    fieldName={"Дата*"}>
                                    <OrderInput placeholder={""} keyField={"date"} typeInput={"date"} val={orderForm.data.date} />
                                </InputField>
                                <InputField wrapperClass={`bx-shadow d-f al-end w-100p pd-20 `} className={"w-100p"}
                                    inputId={"s"}
                                    fieldName={"Имя*"}>
                                    <OrderInput placeholder={"Иван"} keyField={"name"} val={orderForm.data.name} />
                                </InputField>
                            </div>
                            <div className={`f-row-betw gap-20 ${styles.orderFields}`}>
                                <InputField wrapperClass={`bx-shadow d-f al-end w-100p pd-20`} className={"w-100p"}
                                    inputId={"s"}
                                    fieldName={"Телефон*"}>
                                    <OrderInput placeholder={"Телефон"} keyField={"phone"} val={orderForm.data.phone} />
                                </InputField>
                                <InputField wrapperClass={`bx-shadow d-f al-end w-100p pd-20`} className={"w-100p"}
                                    inputId={"s"}
                                    fieldName={"Почта*"}>
                                    <OrderInput placeholder={"email@domain.com"} keyField={"email"} val={orderForm.data.email} />
                                </InputField>
                            </div>
                            <div className={`${styles.peoples} f-row-betw`}>
                                <div className="d-f al-center gap-10">
                                    <div className={`fw-6 ${styles.peopleCountLabel}`}>Кол-во участников</div>
                                    <div className="d-f al-center gap-10">
                                        <div onClick={handleMinusPeople} className="f-c-col w-content">
                                            <ArrowShortIcon />
                                        </div>
                                        <div className={`${styles.peopleCount} f-c-col`}>
                                            <input value={countPeople} readOnly={true} type="text" />
                                        </div>
                                        <div onClick={handlePlusPeople} style={{ transform: "rotateZ(180deg)" }} className="f-c-col w-content">
                                            <ArrowShortIcon />
                                        </div>
                                    </div>
                                </div>
                                <p className={styles.orderMaxCountText}>Максимальное кол-во участников: {props.people_limit}</p>
                            </div>
                        ...
                        <div className="f-row-betw">
                            ...
                            {
                                orderStatus.sending ? <CustomSpinner height={30} width={30} /> :
                                    <div className="f-column gap-5 w-100p">
                                        <UnfilledButton disabled={orderForm.disabledBtn || Object.keys(orderForm.errors).length > 0} onClick={orderSend} className={`w-100p f-05 ${styles.orderButton}`} title={"Оплатить"} />
                                        {orderStatus.success === false ? <div className="errColor txt-center">Не удалось забронировать</div> : null}
