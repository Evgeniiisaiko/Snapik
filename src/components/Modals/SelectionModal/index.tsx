...
import { FormChangeValByKey, handleSelectionForm, SelectionFormType, setSelectionForm } from "../../../features/forms/formsSlice";
...
const SelectionInput: FC<SelectionInputProps> = ({ keyField, placeholder, val }) => {
    ...
    if (keyField == "phone") {
        return <InputMask value={val} mask={"+9(999)999-99-99"} placeholder={"+7"} onChange={e =>
            dispatch(handleSelectionForm({
                keyField: keyField,
                val: e.target.value
            }))} />
    }
    return <input onChange={e => dispatch(handleSelectionForm({
        keyField: keyField,
        val: e.target.value
        ...
        <ShadowWrapper onClick={handleModal}>
            <ModalBody className={`${styles.selectionWindow} f-column-betw`}>
                ...
                <div className={`${styles.form} f-1 f-row-betw gap-20`}>
                    <InputField mistake={!selection.isHiddenErrsValid ? selection.errors?.phone : ""}
                        wrapperClass={`bx-shadow d-f al-end w-100p pd-20`} className={"w-100p"} inputId={"s"}
                        fieldName={"Телефон*"}>
                        <SelectionInput placeholder={"+7"} keyField={"phone"} val={selection.data.phone} />
                    </InputField>
                    <InputField mistake={!selection.isHiddenErrsValid ? selection.errors?.name : ""}
                        wrapperClass={`bx-shadow d-f al-end w-100p pd-20`} className={"w-100p"} inputId={"s"}
                        fieldName={"Имя*"}>
                        <SelectionInput placeholder={"Иван"} keyField={"name"} val={selection.data.name} />
                    </InputField>

                </div>
                <FilledButton disabled={selection.disabledBtn} className={`${styles.btn} w-100p f-1`} title={"Запросить подбор"} />
            </ModalBody>
        </ShadowWrapper>
    );
};
