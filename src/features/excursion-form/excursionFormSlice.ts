...
import { ChangePasswordRules } from '../../config/validation/forms.rules';
...
import { generateCategories } from '../../utils/factories/categoryFactory';
import { CityApi } from '../../types/cities.types';
import { generateCities } from '../../utils/factories/cityFactory';
import { ExcursionApi, ExcursionKind, ExcursionsPayment, ExcursionTransport } from '../../types/excursions.types';
import { HasID } from '../../types/common.types';
import { UserApi } from '../../http/api/user.api';
import { EditAvatarRequest } from '../../types/api/user.api.types';
...
import { allAges, allCostTypes, allDurations, allPeopleCounts } from './allowedData';
...
export const excursionCreate = createAsyncThunk(
    ...
    async (request: {
        ...
        const res: ExcursionApi = await handleTokenRefreshedRequest(FetchApi.CreateExcursion, request.form)

        if (res.id) {
            const reqLocations: LocationsCreateRequest = [...request.data.locations.map(item => {
                return {
                    name: item,
                    description: item,
                    location: item,
            ...
            const reqDetails: DetailsCreateRequest = [...request.data.details.map(item => {
                return {
                    text: item,
                    excursion: res.id
                }
            })]
            const reqImportant: DetailsCreateRequest = [...request.data.important.map(item => {
                return {
                    text: item,
                    excursion: res.id
                }
...
export const getExKinds = createAsyncThunk(
    'all/ex-kinds/get',
    async (_, { dispatch }) => {
...
export const ExcursionFormSlice = createSlice({
    ...
    reducers: {
        ...
        editDetail: (state, action: PayloadAction<{
            index: number,
            val: string
        }>) => {
        ...
        editNeedKnow: (state, action: PayloadAction<{
            index: number,
            val: string
        }>) => {
        ...
        addGalleryItem: (state, action: PayloadAction<{
            src: string,
            file: File | null
        }>) => {
        ...
        handleTextExcursionForm: (state, action: PayloadAction<FormChangeValByKey<TextDataExcursion>>) => {
            const key = action.payload.keyField
            const val = action.payload.val

            const tempTextDataExcursionForm: TextDataExcursion = JSON.parse(JSON.stringify(state.textData))
            tempTextDataExcursionForm[key] = val

            state.textData = tempTextDataExcursionForm
        },
        editGalleryItem: (state, action: PayloadAction<{
            index: number,
        ...
        editLocation: (state: ExcursionFormState, action: PayloadAction<{
            index: number,
            val: string
        }>) => {
        ...
        deleteLocation: (state: ExcursionFormState, action: PayloadAction<number>) => {
            state.data.locations = state.data.locations.filter((item, index) => index !== action.payload)
        },
        deleteGalleryItem: (state: ExcursionFormState, action: PayloadAction<number>) => {
            state.data.gallery = state.data.gallery.filter((item, index) => index !== action.payload)
        },
        resetForm: (state) => {
            state.cost = ""
            state.main_photo = {
