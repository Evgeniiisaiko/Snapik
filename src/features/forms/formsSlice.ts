...
import { AccessUserLoginData, AccessUserRegisterData, ChangePasswordData, OrderFormData, SelectionExcursionData, ToGuideData, UserData } from "../../types/user.types";
import { validate } from "../../utils/validator";
import { authRules, ChangePasswordRules, MIN_DESCRIPTION_GUIDE_COUNT, orderRules, registerRules, selectionRules, ToGuideRules } from "../../config/validation/forms.rules";
import { isValidUrl } from '../../utils/checkValidURL';


export type RegisterFormType = AccessUserRegisterData & UserData & {
    description?: string
}
...
export const FormsSlice = createSlice({
    ...
    initialState,
    reducers: {
        handleRegisterForm: (state, action: PayloadAction<FormChangeValByKey<RegisterFormType>>) => {
            const key = action.payload.keyField
            const val = action.payload.val

            const tempRegisterForm: RegisterFormType = JSON.parse(JSON.stringify(state.registerForm.data))
            ...
            const { first_name, last_name, password, phone, email, description, is_gid } = tempRegisterForm
            const descriptionStr = String(description)
            const isNotFilled = (is_gid ? descriptionStr.length < MIN_DESCRIPTION_GUIDE_COUNT : false) || !first_name || !last_name || !password || (phone.includes("_") || phone.length < 16) || !email

            state.registerForm = {
                data: { ...tempRegisterForm },
                errors: !isNotFilled ? validate<RegisterFormType>(tempRegisterForm, registerRules) : {},
        ...
        handleSelectionForm: (state, action: PayloadAction<FormChangeValByKey<SelectionFormType>>) => {
            const key = action.payload.keyField
            const val = action.payload.val

            const tempSelectionForm: SelectionFormType = JSON.parse(JSON.stringify(state.selectionForm.data))
            ...
            state.selectionForm = {
                ...
                errors: !isNotFilled ? validate<SelectionFormType>(tempSelectionForm, selectionRules) : {},
                disabledBtn: isNotFilled,
                isHiddenErrsValid: true
            }

        },
        handleChangePasswordForm: (state, action: PayloadAction<FormChangeValByKey<ChangePasswordData>>) => {
            const key = action.payload.keyField
            const val = action.payload.val

            const tempChangePasswordForm: ChangePasswordData = JSON.parse(JSON.stringify(state.changePasswordForm.data))
            ...
            state.changePasswordForm = {
                ...
                errors: !isNotFilled ? validate<ChangePasswordData>(tempChangePasswordForm, ChangePasswordRules) : {},
        ...
        handleToGuideForm: (state, action: PayloadAction<FormChangeValByKey<ToGuideData>>) => {
            ...
            const incorrectURL = video_presentation.length > 0 ? !isValidUrl(video_presentation) : false
