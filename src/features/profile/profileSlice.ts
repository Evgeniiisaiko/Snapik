...
import { ChangePasswordData, ProfileData, ToGuideData, UserData } from '../../types/user.types';
import { AxiosResponse } from 'axios';
import { UserApi } from '../../http/api/user.api';
import { handleTokenRefreshedRequest } from '../../utils/handleThunkAuth';
import { EditAvatarRequest, EditProfileRequest, GetProfileResponse, MakeGuideAccountRequest } from '../../types/api/user.api.types';
import { FormChangeValByKey } from '../forms/formsSlice';
import { MIN_DESCRIPTION_GUIDE_COUNT, selectionRules } from '../../config/validation/forms.rules';
import { validate } from '../../utils/validator';
import { OrderApi } from '../../types/order.types';
import { generateOrders } from '../../utils/factories/orderFactory';
...
import { handleAvatarEditor } from '../modals/modalsSlice';
import { resetEditAvatar } from '../edit-avatar/main';
...
export const getOrderById = createAsyncThunk(
    ...
    async (request: OrderByIdRequest, { dispatch }) => {
        const res: AxiosResponse<OrderApi> = await handleTokenRefreshedRequest(UserApi.GetOrderById, request)
        return res.data
    }
)
...
export const ProfileSlice = createSlice({
    ...
    reducers: {
        ...
        handleProfileForm: (state, action: PayloadAction<FormChangeValByKey<ProfileData>>) => {
            ...
            const ignoreForUser: Array<keyof ProfileData> = [
                "description"
            ]
                ...
                if (ignoreForBoth.includes(key)) {
                    if (state.data.is_gid) {
                        if (validKey == "video_presentation") {
                            const incorrectURL = tempProfileForm[validKey].length > 0 ? !isValidUrl(tempProfileForm[validKey]) : false
                            return incorrectURL
                        }
                        return false
                    }
                    ...
                    if (validKey == "description") {
                        return String(tempProfileForm[validKey]).length < MIN_DESCRIPTION_GUIDE_COUNT
                    }
                    return String(tempProfileForm[validKey]).length == 0
...
export const { setLoginLoading, handleProfileForm, resetChangePasswordStatus, resetEditAvatarStatus } = ProfileSlice.actions
