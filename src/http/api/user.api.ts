...
import { AccessUserLoginData, AccessUserRegisterData, AuthResponse, ChangePasswordData, JWT, UserData } from "../../types/user.types";
import { PATHS, api, authApi } from "../instance";
import { EditAvatarRequest, EditProfileRequest, EditProfileResponse, GetProfileResponse, LoginRequest, LoginResponse, MakeGuideAccountRequest, RefreshRequest, RefreshResponse, RegisterRequest, RegisterResponse } from "../../types/api/user.api.types";
...
export class UserApi {
    static async Registration(req: RegisterRequest): Promise<RegisterResponse> {
        const res: AxiosResponse<RegisterResponse> = await api.post(PATHS.USER_REGISTER, { ...req })
        return res
    }
    ...
    static async RefreshToken(req: RefreshRequest): Promise<RefreshResponse> {
        const res: AxiosResponse<RefreshResponse> = await api.post(PATHS.USER_TOKEN_REFRESH, { ...req })
        return res.data;
    }
    ...
    static async GetOrderById(req: { id: number }): Promise<AxiosResponse<OrderApi>> {
        const res: AxiosResponse<OrderApi> = await authApi.get(`${PATHS.ORDER_GET_BY_ID}/${req.id}`)
        if (!res.data) {
            throw res
        }
        return res
    }
    static async EditUser(req: EditProfileRequest | MakeGuideAccountRequest): Promise<AxiosResponse<EditProfileResponse>> {
    ...
    static async ChangePassword(req: ChangePasswordData): Promise<AxiosResponse<any>> {
        const res: AxiosResponse<ChangePasswordData> = await authApi.post(PATHS.USER_CHANGE_PASSWORD, req)
        if (!res.data) {
            throw res
        }
        return res
