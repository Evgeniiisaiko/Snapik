...
import { excursionCreate, handleSelectedCostType, setCost } from "../../../../../../features/excursion-form/excursionFormSlice"
...
export const CostStep = () => {
    ...
    const { data, selectData, allowedCostTypes, cost, textData, allowedExcursionKinds, allowedAges, allowedLoadings, allowedCategories, allowedDurations, allowedPeoplesCounts, allowedCities, allowedTransports, main_photo, sending } = useAppSelector(state => state.excursionForm)
    const { profile } = useAppSelector(state => state)


    const selectDataIsValid = Object.keys(selectData)
        .some(key => {
            const typedKey = key as keyof typeof selectData
            return selectData[typedKey] !== 0
        })
    const textDataIsValid = Object.keys(textData)
        .some(key => {
            const typedKey = key as keyof typeof textData
            return textData[typedKey].length > 0
        })
    const arraysDataIsNotValid = Object.keys(data)
        .some(key => {
    ...
    const validFormData = selectDataIsValid && textDataIsValid && costIsValid && !arraysDataIsNotValid
    ...
    const handleCreateExcursion = () => {
        ...
        const peoplesCountVal = allowedPeoplesCounts.find(item => item.id === selectData.peoplesCount)?.val
    ...
    return (
        <div className={"f-column gap-25"}>
            ...
            <div className="formArea gap-30 f-column">
                ...
                <SelectInput loading={allowedLoadings.cost_types} labelText={"Тип стоимости"} current={selectData.costType} items={allowedCostTypes}
                    selectHandler={(id) => dispatch(handleSelectedCostType(id))} />
                <InputField inputType={"number"} wrapperClass={"d-f al-end w-100p pd-20"} className={"w-100p"}
                    inputId={"s"}
                    fieldName={"Стоимость на руки (руб.)"}>
                    <input type={"text"} min={100} value={cost} onChange={e => dispatch(setCost(e.target.value))} placeholder={"2500"} className={`f-1`} />
                </InputField>
            ...
            <div className="f-column w-100p gap-10">
                <div className="f-row-betw">
                    ...
                    {
                        sending.loading ? <CustomSpinner height={20} width={20} /> : <UnfilledButton disabled={!validFormData} onClick={handleCreateExcursion} title={"Создать экскурсию"} />
                    }

                </div>
                {
                    !validFormData ?
                        <p className="errColor txt-right">Не заполнены обязательные поля. Проверьте все пункты!</p> : null
                }
