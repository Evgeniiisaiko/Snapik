...
import { addGalleryItem, editGalleryItem, deleteGalleryItem } from "../../../../../../features/excursion-form/excursionFormSlice"
        ...
        <div className={"f-column gap-25"}>
            ...
            <div className="f-column gap-30">
                <div className={"f-column gap-5 w-content"}>
                    ...
                    <label htmlFor={"galleryInput"} onClick={() => {
                    }} className={"w-content"}>
                        <UnfilledButton asBlock={true} title={"+ Добавить фото"} />
                    </label>
                    <input onChange={handleAddGalleryItem} accept={"image/png, image/jpeg"} id={"galleryInput"}
                        type="file" className={"d-f hidden"} />
                ...
                <div className={`${styles.galleryItems} f-row-betw gap-30 flex-wrap`}>
                    {gallery.map((item, index) => (
                        <div className={`${styles.galleryItem} w-100p f-column gap-15`}>
                            <input id={`galleryInputEdit-${index}`} onChange={e => handleEditImageGalleryItem(e, index)}
                            ...
                            <div className={`${styles.imageWrapper} w-100p  h-100p`}>
                                <div style={{ backgroundImage: `url(${item.src})` }}
                                    ...
                                    <label htmlFor={`galleryInputEdit-${index}`} className={`f-c-col ${styles.edit}`}>
                                        <EditIcon />
                                    </label>
                                    <div onClick={() => dispatch(deleteGalleryItem(index))} className={`f-c-col ${styles.delete}`}>
                                        <DeleteIcon strokeColor={"red"} height={18} />
                                    </div>
                                </div>
                            </div>
                            <InputField defaultValue={item.description} wrapperClass={"d-f al-end w-100p pd-20"}
                                className={"w-100p"} inputId={"s"}
                                fieldName={"Описание к экскурсии"}>
                                <input placeholder={"Название..."} onChange={e => dispatch(editGalleryItem({ index, prop: "description", val: e.target.value }))} className={`f-1`} />
                            </InputField>
