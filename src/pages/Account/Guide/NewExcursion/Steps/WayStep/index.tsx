...
import { addLocation, editLocation, deleteLocation } from "../../../../../../features/excursion-form/excursionFormSlice"
                ...
                </div>
                {
                    travelRoutes.map((item, index) => (
                        <div className="locationContainer f-row-betw gap-50">
                            <InputField wrapperClass={"d-f al-end w-100p pd-20 "} className={"w-100p"}
                                inputId={`r_${index}`}
                                fieldName={`Локация ${index + 1}`}>
                                <input onChange={e => dispatch(editLocation({ index, val: e.target.value }))} value={item}
                                    placeholder={"Локация экскурсии..."} className={`f-1`} />
                                <div onClick={() => dispatch(deleteLocation(index))}
                                    className={"inputRight cur-pointer"}>Удалить
                                </div>
                            </InputField>
                        </div>
