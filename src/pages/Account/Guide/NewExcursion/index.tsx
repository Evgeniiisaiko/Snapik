...
const ExcursionFormContainer = () => {
    ...
    }, [sending.loading, sending.success])
    ...
    return (
        <ExcursionFormContext.Provider value={{ handlePrevStep, handleNextStep }}>
            <div className="f-column ">
                <SidePartLayout afterContent={
                    <div ref={selectInputRef} className={`${styles.formExcursionMenuBtn} bx-shadow d-f al-center gap-10 p-rel`}>
                        <div onClick={toggleMenuOpened} className={"d-f al-center gap-10"}>
                            <FormMenuIcon strokeColor={"#D900b6"} height={28} width={28} />
                            <p>Меню</p>
                        </div>

                        <div ref={dropdownRef} className={`${styles.formExcursionMenu} ${menuExcursionOpened ? styles.formExcursionMenuOpened : null}  t-transform-4 bg-white f-column gap-5 pd-10 p-abs `}>
                            {
                                steps.map((step, index) => (
                                    <div onClick={index !== currentStep ? () => handleSelectStep(index) : undefined} className={`${styles.formExcursionMenuItem} ${index === currentStep ? "colorPurple" : null}`}>{step.name}</div>
                                ))
                            }
                        </div>
                    </div>

                } backLink={"/profile"} defaultTitle={"Новая экскурсия"} backText={"В личный кабинет"}
                    tabsContent={<NewExcursionBar currentTab={currentStep} setCurrentTab={setCurrentStep}
                        tabs={steps.map(step => {
