...
import { handleOrder, handleSelection } from "../../features/modals/modalsSlice";
import { log } from 'console';
import { addToSeen } from '../../features/seen/seenSlice';
import OrderModal from '../../components/Modals/Order';
import { createReview, getExcursion, resetData } from '../../features/excursion-page/excursionPageSlice';
import { PagePreloader } from '../../components/PagePreloader';
import ExcursionsSlider from '../../components/Sections/PopularExcursionsSection/slider';
import { add } from '../../features/favorites/favoritesSlice';
...
export const ReviewExcursionForm = () => {
    ...
    return (
        <>
            ...
            <form
                className={`${styles.reviewForm} f-column gap-30`}
                action=""
            >
                    ...
                    ></textarea>
                    {
                        !validReviewText ?
                            <p style={{ fontSize: 12 }} className={"errColor"}>
                                Минимум {MIN_REVIEW_COUNT} символов ({reviewVal.length}/{MIN_REVIEW_COUNT})
                            </p> :
                            null
                    }
                </div>
                <div className="d-f al-center gap-10">
                    {
                        Array(5).fill("").map((item, index) => (
                            <div onClick={() => setStars(index + 1)} className={"cur-pointer"}>
                                <StarBordered height={30} width={30} fill={(index + 1) > stars ? "lightgray" : undefined} />
                            </div>
                        ))
                    }
                </div>
                <div className="d-f al-center gap-10">
                    <UnfilledButton onClick={sendCreateReview} disabled={!validReview || reviewForm.sending} className={styles.button} title={'Оставить отзыв'} />
...
const Excursion = () => {
    ...
    const sliderWelcomeRef = useRef<SwiperProps>()
    const { similar, ex_data, loadings, photos, locations, reviews, reviewForm, pageStatus } = useAppSelector(state => state.excursionPageData)
            ...
            <div className="f-column gap-50 p-rel">
                    ...
                    >
                        {
                            ...
                            photos.length == 0 ?
                                <SwiperSlide>
                                    <div style={{
                                        backgroundImage: ex_data.main_photo && pageStatus.finded ? `url(${ex_data.main_photo})` : undefined,
                                        backgroundColor: ex_data.main_photo && pageStatus.finded ? undefined : "gray",
                ...
                >
                    {pageStatus.finded ?
                        <div className="h-100p c-white p-rel left-0">
                            ...
                            >
                                ...
                                <div className="wrapper">
                                    <div className={styles.welcomeExcursionTitle}>{ex_data.title}</div>
                                </div>

                                {
                                    photos.length > 1 ?
                                        <div className={`${styles.sliderExcursion} pd-20 f-row-betw w-100p`}>
                                            <div className={`${styles.slideBtn} cur-pointer f-c-col`} onClick={handlePrev}>
                                                <ArrowShortLeftIcon height={16} width={16} />
                                            </div>
                                            <div className={`${styles.slideBtn}  cur-pointer f-c-col`} onClick={handleNext}>
                                                <ArrowShortRightIcon height={16} width={16} />
                                            </div>
                                        </div> : null
                                }

                                <div className={`${styles.excursionBottom} wrapper f-row-betw flex-wrap gap-20`}>
                                    <div className={`${styles.breadCrumbs} al-center gap-20 d-f`}>
                                        {pageStatus.finded ?
                                            <>
                                                <Link to={'/'}>Главная</Link>
                                                <BracketRight height={14} width={18} />
                                                <Link to={`/city/${ex_data.city.id}`}>{ex_data.city.name}</Link>
                                                <BracketRight height={14} width={18} />
                                                <div>{ex_data.title}</div>
                                            </> : null
                                        }
                                    </div>
                                    {pageStatus.finded ? <div className="f-row-betw w-100p">
                                        <div className={`${styles.excursionInfo} d-f al-center gap-50`}>
                                            <div className={`${styles.rating} d-f al-center gap-15`}>
                                                <StarBordered height={30} width={30} />
                                                <p>{ex_data.rating || "Нет оценок"}</p>
                                            </div>
                                            <div
                                                onClick={!isFavorite ? handleToFavorite : handleDeleteFavorite}
                                        ...
                                        {
                                            photos.length > 1 ?
                                                <div className={`${styles.sliderArrowsMobile} pd-20 d-f gap-10 `}>
                                                    <div className={`${styles.slideBtn} cur-pointer f-c-col`} onClick={handlePrev}>
                                                        <ArrowShortLeftIcon height={16} width={16} />
                                                    </div>
                                                    <div className={`${styles.slideBtn}  cur-pointer f-c-col`} onClick={handleNext}>
                        ...
                        </div> : <div className="f-c-col h-100p gap-30 pd-0-20">
                            <h1 style={{ fontSize: 32 }} className="c-white txt-center">Экскурсия не найдена</h1>
                            <Link to={"/"}>
                    ...
                    <section className={`d-n ${styles.excursionNavigation}`}>
                        <div className="wrapper">
                            <div className="block  f-row-betw">
                                <div className={`${styles.pageLink} cur-pointer gap-10 al-center d-f`}>
                                    <ArrowLeft />
                                    <p>Предыдущая</p>
                                </div>
                                <div className={`${styles.pageLink} cur-pointer gap-10 al-center d-f`}>
                        ...
                        <div className={`${styles.wrapperExcursions} wrapper`}>
                            <div className={`${styles.block} f-column gap-50`}>
                                <div className={`${styles.top} gap-20 f-row-betw`}>
                                    ...
                                    <div className={`${styles.price} flex-wrap d-f al-center gap-20`}>
                                        <div className={`${styles.priceInfo} price f-column`}>
                                            <h3>{ex_data.cost} ₽</h3>
                                            <p>{ex_data.payment_type.name}</p>
                                        </div>
                                        <UnfilledButton onClick={handleOrderExcursion} className={styles.orderBtn} title={'Забронировать'} />
                                    </div>
                                </div>
                                <div className="f-row-betw gap-20 flex-wrap">
                                    <div className={`${styles.excursionFeature} d-f al-center gap-20`}>
                                ...
                                <div className={`${styles.infoBlock} f-column gap-70`}>
                                    <div className={`${styles.pointsBlock} f-column gap-40`}>
                                        ...
                                        <div className={`${styles.points} f-column gap-30`}>
                                            <div className={`${styles.paragraphBlock} f-column gap-15`}>
                                            ...
                                            <div className={`${styles.paragraphBlock} f-column gap-15`}>
                                                ...
                                                <div className="f-column gap-20">
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            {ex_data.guide.profile_photo === null ? <AvatarDefaultIcon height={40} width={40} /> : null}
                                                        </div>
                                                        <div className={styles.person}>
                                                            <div className="d-f al-center gap-15">
                                                                <div className={styles.name}>{ex_data.guide.first_name || "Гид"} {ex_data.guide.last_name}</div>
                                                                {/* <div
                                                          ...
                                                          >
                                                            ...
                                                            {ex_data.guide.description.length ? <div className={styles.description}>
                                                    ...
                                                    <a href={`tel:${ex_data.guide.phone}`}>
                                                        <WhiteButton className={styles.messageGuide}>Позвонить гиду</WhiteButton>
                                                    </a>
                                            ...
                                            <div className={`${styles.paragraphBlock} f-column gap-15`}>
                                                ...
                                                <div className={`${styles.listBlock} flex-column gap-10`}>
                                                    {
                                                        ex_data.extra_details !== undefined && ex_data.extra_details.length > 0 ?
                                                            ex_data?.extra_details.map((item, index) => (
                                                                <div key={index} className={`${styles.listItem} d-f gap-5`}>
                                                                    {item.text}
                                                                </div>
                                                            )) :
                                                            <div className={styles.emptySection}>
                                                                <p className="emptyText">Пункты не заполнены.</p>
                                                            </div>
                                                    }
                                                </div>
                                            </div>
                                            <div className={`${styles.paragraphBlock} f-column gap-15`}>
                                                ...
                                                <div className={`${styles.listBlock} flex-column gap-10`}>
                                                    {
                                                        ex_data.organizational_details !== undefined && ex_data.organizational_details.length > 0 ?
                                                            ex_data?.organizational_details.map((item, index) => (
                                                                <div key={index} className={`${styles.listItem} d-f gap-5`}>
                                                                    {item.text}
                                                                </div>
                                                            )) : <div className={styles.emptySection}>
                                        ...
                                        <div className={`${styles.locations} f-column gap-10`}>
                                            {
                                                locations.length > 0 ?
                                                    ...
                                                    <div className={styles.emptySection}>
                                                        <p className="emptyText">Локации не заполнены.</p>
                                                    </div>
                                            }
                                        </div>
                                    </div>
                                    <div className={`${styles.pointsBlock}  f-column gap-40`}>
                                        ...
                                        <div className={`${styles.reviews} f-column gap-20`}>
                                            {reviews.length > 0 ?
                                                <ReviewsSlider reviews={reviews} /> : <div className={styles.emptySection}>
                                            ...
                                            {linkCopied !== null ?
                                                linkCopied ?
                                                    <div className="d-f al-center gap-10">
                                                        ...
                                                        <p style={{ fontSize: 14 }}>Ссылка скопирована</p>
                                                    </div> :
                                                    <p style={{ fontSize: 14 }}>Не удалось скопировать ссылку</p> :
                                                <WhiteButton onClick={copyExcursionLink} className={styles.shareBtn}>
                                                    <div className={`${styles.share} f-row-betw gap-10`}>
                                                        <ShareIcon />
                                    ...
                                    <div className={`${styles.pointsBlock} f-column gap-40`}>
                                        ...
                                        {similar.length > 0 ?
                                            <ExcursionsSlider mbClassName={styles.similarItem} sliderMargin={12} excursions={similar} />
                                            : <div className={styles.emptySection}>
                                                <p className="emptyText">Похожих экскурсий не найдено</p>
                                            </div>
                                        }
                                    </div>
                                </div>
