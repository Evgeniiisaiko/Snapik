...
const Search = () => {
    ...
    useEffect(() => {
        dispatch(getSearchedExcursions({
    ...
    }, [search.category, search.val, search.filterVariant, search.city])
                    ...
                    <div className="wrapper">
                        <div className="searchBlock f-column gap-30">
                            ...
                            <div className="f-column sectionBlock">
                                <h2 className="section-title blured-entrance">Найдите экскурсию мечты!</h2>
                                <div className={`${styles.searchPageTop} f-row-betw flex-wrap gap-20`}>
                                    <div className={`${styles.searchLeft} d-f al-center gap-60 flex-wrap f-1 w-100p`}>
                                        <div className={`d-f al-center gap-20 flex-wrap w-100p ${styles.searchInputs}`}>
                                            <div className={`${styles.searchWrapper} gap-30 f-row-betw`}>
                                                <input
                                                    ...
                                                    type="text"
                                                />
                                                <div onClick={handleSearch} className="f-c-col w-content">
                                                    <SearchIcon strokeColor={'#D900B6'} height={28} width={28} />
                                                </div>

                                            </div>
                                            <SelectInput defaultText={"Все категории"} classNameRefBlock={styles.searchCategoryWrapper} className={styles.searchCategoryInput} current={search.category || (search.categoriesResults.length > 0 ? search.categoriesResults[0].id : 0)}
                                                items={[{ id: 0, name: "Все категории" }, ...search.categoriesResults]} selectHandler={(ctgId) => dispatch(setCategory(ctgId))} />
                                            <SelectInput defaultText={"Все города"} classNameRefBlock={styles.searchCategoryWrapper} className={styles.searchCityInput} current={search.allCities.length > 0 ? search.allCities[0].id : 0}
                                                items={[{ id: 0, name: "Все города" }, ...search.allCities]} selectHandler={(cityId) => dispatch(setSearchCity(cityId))} />

                                            <div ref={filterBlockRef} className={`${styles.filter} p-rel`}>
                                                <div onClick={handleSortIsOpened} className={"cur-pointer d-f al-center gap-10"}>
                                                    <FilterSearchIcon width={30} height={30} />
                                                    <p>{filterVariants.find(item => item.id === search.filterVariant)?.name}</p>
                                                </div>
                                                <SelectDropDown current={search.filterVariant} selectHandler={(current) => handleFilter(current)} className={`${sortIsOpened ? styles.searchFilterOpened : styles.searchFilterIdle} t-opacity-visible-transform-3`} items={filterVariants} />
                                            </div>
                                        </div>
                                    </div>
                                    <p className={styles.finded}>Найдено: {search.results.length}</p>
                                </div>
                            </div>
                            ...
                            {search.loadings.excursions ?
                                ...
                                search.results.length ?
                                    <List
                                        ...
                                        renderItem={(ex) => {
                                            return (
                                                <ExcursionCard classNameMobile={styles.searchMobileCard} {...ex as ExcursionProps}
                                                />
                                            );
                                        }}
                                    /> : <p className="emptyText">
                                        Экскурсий по вашему запросу не найдено.
                                    </p>
                            }
                        </div>
                    </div>
